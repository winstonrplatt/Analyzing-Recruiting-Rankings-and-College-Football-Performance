...
The data import and wrangling section require the download of approximately 40 CSV files, since we are pulling data from many sources. For ease of grading, instead of requiring the download of all of these files, we've exported the final filtered_data.csv file, which contains the necessary data after the wrangling steps.

To load the data, please run the following code chunks at the beginning of the analysis:
...
Once the data is loaded, you can proceed with the code in the EDA section as usual, bypassing the need to run the code for the data import and wrangling steps.
